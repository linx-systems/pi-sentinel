...
test.describe('Extension Popup Access', () => {
    test('can access extension popup (requires extension ID)', async ({page}) => {
        ...
        await expect(page.locator('text=PiSentinel')).toBeVisible({timeout: 5000});
    });
});
...
test.describe('Extension State Verification', () => {
    test('extension storage is accessible', async ({context}) => {
        ...
        const hasBrowserApi = await page.evaluate(() => {
            return typeof (window as any).browser !== 'undefined' ||
